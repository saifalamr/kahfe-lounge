      ...
      <div className="kahfe-modal" onClick={e=>e.stopPropagation()} style={{ width:'100%', maxWidth:560, margin:'0 auto', background:'#141414', borderRadius:20, maxHeight:'85vh', overflowY:'auto', border:'1px solid rgba(201,168,76,.3)', boxShadow:'0 20px 60px rgba(0,0,0,.6)', animation:'modalPopIn .3s cubic-bezier(.18,.84,.26,1) both' }}>
        ...
        <div style={{ padding:'16px 20px' }}>
          <div style={{ display:'flex', gap:10, marginBottom:16 }}>
            <div style={{ flex:1, background:'#1A1A1A', borderRadius: 8, padding:'12px', textAlign:'center', border:'1px solid rgba(201,168,76,.2)' }}>
            ...
            <div style={{ flex:1, background:'#1A1A1A', borderRadius: 8, padding:'12px', textAlign:'center', border:'1px solid rgba(231,76,60,.25)' }}>
          ...
          {report.prevLabel && (report.revenueDeltaPct !== null || report.ordersDeltaPct !== null) && (
            <div style={{ color:'#8A8A8A', fontSize:12, marginBottom:16, textAlign:'center' }}>
          ...
          {paymentRows.length > 0 && (
            <>
              <div style={{ color:'#8A8A8A', fontSize:11, letterSpacing:1, marginBottom:10 }}>ÖDEME YÖNTEMİ DAĞILIMI</div>
              {paymentRows.map((r, i) => {
                const pct = paymentTotal > 0 ? (r.value / paymentTotal) * 100 : 0
                return (
                  <div key={i} style={{ marginBottom:8 }}>
                    <div style={{ display:'flex', justifyContent:'space-between', fontSize:13, marginBottom:4 }}>
                      <span style={{ color:'#F0EDE8' }}>{r.label}</span>
                      <span style={{ color:r.color, fontWeight:700 }}>{formatTL(r.value)} ₺ · %{pct.toFixed(0)}</span>
                    </div>
                    <div style={{ background:'#2A2A2A', height:6, borderRadius:3, overflow:'hidden' }}>
          ...
          {activeHours.length > 0 && (
            <>
              <div style={{ color:'#8A8A8A', fontSize:11, letterSpacing:1, marginBottom:10 }}>
              ...
              <div style={{ display:'flex', alignItems:'flex-end', gap:2, height:64, marginBottom:16 }}>
                {activeHours.map((h: any, i: number) => (
                  <div key={i} title={`${String(h.hour).padStart(2,'0')}:00 — ${h.orders} sipariş`} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'flex-end', height:'100%' }}>
                    ...
                    <div style={{ color:'#8A8A8A', fontSize:8, marginTop:3 }}>{h.hour}</div>
                  </div>
                ))}
              </div>
            </>
