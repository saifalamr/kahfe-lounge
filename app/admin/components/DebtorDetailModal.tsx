...
export default function DebtorDetailModal({ debtor, stats, debtPaymentAmount, onDebtPaymentAmountChange, manualDebtAmount, onManualDebtAmountChange, manualDebtNote, onManualDebtNoteChange, onRecordPayment, onAddManualDebt, onClose }: {
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:200, background:'rgba(0,0,0,.9)', backdropFilter:'blur(6px)', display:'flex', alignItems:'center', justifyContent:'center', padding:'20px' }} onClick={onClose}>
      <div className="kahfe-modal" onClick={e=>e.stopPropagation()} style={{ width:'100%', maxWidth:520, margin:'0 auto', background:'var(--a-bg2)', borderRadius:20, maxHeight:'85vh', overflowY:'auto', border:'1px solid rgba(201,168,76,.3)', boxShadow:'0 20px 60px rgba(0,0,0,.6)', animation:'modalPopIn .3s cubic-bezier(.18,.84,.26,1) both' }}>
        <div style={{ padding:'20px', borderBottom:'1px solid #2A2A2A', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <div>
            <div style={{ color:'var(--a-text)', fontWeight:700, fontSize:18, fontFamily:"'Bricolage Grotesque', sans-serif" }}>{debtor.name}</div>
            {debtor.phone && <div style={{ color:'var(--a-text2)', fontSize:12, fontFamily:"'IBM Plex Mono', monospace" }}>{debtor.phone}</div>}
          </div>
        ...
        <div style={{ padding:20 }}>
          <div style={{ display:'flex', gap:10, marginBottom:20 }}>
            <div style={{ flex:1, background:'var(--a-bg1)', border:'1px solid #2A2A2A', padding:14, textAlign:'center' }}>
              <div style={{ color:'var(--a-text2)', fontSize:11 }}>TOPLAM BORÇ</div>
              <div style={{ color:'var(--a-text)', fontWeight:800, fontSize:18, fontFamily:"'IBM Plex Mono', monospace" }}>₺{formatTL(stats.borc)}</div>
            </div>
            <div style={{ flex:1, background:'var(--a-bg1)', border:'1px solid #2A2A2A', padding:14, textAlign:'center' }}>
              <div style={{ color:'var(--a-text2)', fontSize:11 }}>ÖDENEN</div>
              <div style={{ color:'#27ae60', fontWeight:800, fontSize:18, fontFamily:"'IBM Plex Mono', monospace" }}>₺{formatTL(stats.odenen)}</div>
            </div>
            <div style={{ flex:1, background:'var(--a-bg1)', border:'1px solid #2A2A2A', padding:14, textAlign:'center' }}>
              <div style={{ color:'var(--a-text2)', fontSize:11 }}>KALAN</div>
              <div style={{ color: stats.kalan > 0 ? '#e74c3c' : 'var(--a-text2)', fontWeight:800, fontSize:18, fontFamily:"'IBM Plex Mono', monospace" }}>₺{formatTL(stats.kalan)}</div>
            </div>
          </div>
          ...
          <div style={{ display:'flex', gap:8, marginBottom:16 }}>
            <input type="number" value={debtPaymentAmount} onChange={e => onDebtPaymentAmountChange(e.target.value)} placeholder="Ödeme tutarı (₺)"
              style={{ flex:1, height:48, background:'var(--a-bg0)', border:'1px solid #383838', color:'var(--a-text)', padding:'0 14px', fontSize:15, fontFamily:"'IBM Plex Mono', monospace" }} />
          ...
          <div style={{ display:'flex', gap:8, marginBottom:20 }}>
            ...
            <input value={manualDebtNote} onChange={e => onManualDebtNoteChange(e.target.value)} placeholder="Not (opsiyonel)"
              style={{ flex:1, height:48, background:'var(--a-bg0)', border:'1px solid #383838', color:'var(--a-text)', padding:'0 14px', fontSize:14, fontFamily:"'IBM Plex Sans', sans-serif" }} />
          ...
          <div style={{ color:'var(--a-text2)', fontSize:11, textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:10 }}>Hareketler</div>
          {stats.txs.length === 0 && <div style={{ color:'var(--a-text2)', fontSize:13, textAlign:'center', padding:'10px 0' }}>Henüz hareket yok.</div>}
          {stats.txs.map((t: any) => (
            <div key={t.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'10px 0', borderTop:'1px solid #2A2A2A' }}>
              <div>
                ...
                <div style={{ color:'var(--a-text2)', fontSize:11 }}>{new Date(t.created_at).toLocaleString('tr-TR')} {t.note ? `· ${t.note}` : ''}</div>
              </div>
              <div style={{ color:'var(--a-text)', fontWeight:700, fontSize:15, fontFamily:"'IBM Plex Mono', monospace" }}>₺{formatTL(Number(t.amount))}</div>
            </div>
          ))}
        </div>
      </div>
